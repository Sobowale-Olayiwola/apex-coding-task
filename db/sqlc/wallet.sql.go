...
WHERE id = $2
RETURNING id, owner, balance, asset, created_at
`
...
func (q *Queries) CreateWallet(ctx context.Context, arg CreateWalletParams) (Wallet, error) {
	row := q.db.QueryRowContext(ctx, createWallet, arg.Owner, arg.Balance, arg.Asset)
	var i Wallet
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
SELECT id, owner, balance, asset, created_at FROM wallets
WHERE id = $1 LIMIT 1
`
...
func (q *Queries) GetWalletByUsername(ctx context.Context, owner string) (Wallet, error) {
	row := q.db.QueryRowContext(ctx, getWalletByUsername, owner)
	var i Wallet
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
func (q *Queries) GetWalletForUpdate(ctx context.Context, owner string) (Wallet, error) {
	row := q.db.QueryRowContext(ctx, getWalletForUpdate, owner)
	var i Wallet
	err := row.Scan(
		&i.ID,
		&i.Owner,
